#!/usr/bin/env bash
# Build Anka for Linux x86_64 from macOS using Podman.
#
# Usage:
#   ./scripts/build-anka-linux-x86_64-podman.sh
#   ./scripts/build-anka-linux-x86_64-podman.sh --debug
#   ./scripts/build-anka-linux-x86_64-podman.sh --clean
#
# Output:
#   dist/anka-linux-x86_64
#
# The Linux build uses its own target-linux-x86_64/ directory and never
# overwrites the native macOS anka binary.
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            cat <<'EOF'
Build Anka for Linux x86_64 inside Podman.
